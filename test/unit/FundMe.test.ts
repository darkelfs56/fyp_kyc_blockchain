  ...
  : describe("FundMe", function () {
      ...
      describe("withdraw", function () {
        ...
        it("is allows us to withdraw with multiple funders", async () => {
          ...
          assert.equal((await fundMe.s_addressToAmountFunded(accounts[1].address)).toString(), "0")
          assert.equal((await fundMe.s_addressToAmountFunded(accounts[2].address)).toString(), "0")
          assert.equal((await fundMe.s_addressToAmountFunded(accounts[3].address)).toString(), "0")
          assert.equal((await fundMe.s_addressToAmountFunded(accounts[4].address)).toString(), "0")
          assert.equal((await fundMe.s_addressToAmountFunded(accounts[5].address)).toString(), "0")
        })
      })
    })
